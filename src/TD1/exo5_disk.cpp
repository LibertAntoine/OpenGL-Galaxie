...
int main(int argc, char** argv) {
	...
	for (GLuint i = 1; i <= precision; i++) {
		...
		vectDisk.push_back(Vertex2DColor(glm::vec2(0.5f*glm::cos(((i+1)*2.0f*glm::pi<float>()) / precision), 0.5f*glm::sin(((i+1)*2.0f*glm::pi<float>()) / precision)), glm::vec3(1, 1, 1)));
	}

	glBufferData(GL_ARRAY_BUFFER, 3 * precision * (sizeof(Vertex2DColor)), vectDisk.data(), GL_STATIC_DRAW); // Add VBO data
    ...
    while(!done) {
		...
		glBindVertexArray(vao);
		glDrawArrays(GL_TRIANGLES, 0, 3*precision);
		glBindVertexArray(0);
